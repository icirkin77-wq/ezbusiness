import Link from "next/link";

const steps = [
  {
    number: "1",
    title: "Разбираем идею",
    text: "Определяем, чем вы хотите заниматься, для кого создаётся продукт и какую проблему он решает.",
    icon: "💡",
  },
  {
    number: "2",
    title: "Проверяем перспективность",
    text: "Изучаем аудиторию, конкурентов, спрос и основные риски до того, как вы начнёте тратить деньги.",
    icon: "🔎",
  },
  {
    number: "3",
    title: "Считаем экономику",
    text: "Разбираемся с расходами, доходами, ценой продукта и потенциальной прибылью.",
    icon: "📊",
  },
  {
    number: "4",
    title: "Составляем план",
    text: "Превращаем всю информацию в понятную последовательность действий.",
    icon: "🗺️",
  },
  {
    number: "5",
    title: "Начинаем запуск",
    text: "Переходим от рассуждений к конкретным действиям и первым результатам.",
    icon: "🚀",
  },
  {
    number: "6",
    title: "Развиваем",
    text: "Анализируем результаты, исправляем ошибки и ищем следующие точки роста.",
    icon: "📈",
  },
];

export default function HowPage() {
  return (
    <main className="min-h-screen bg-[#0B1220] text-white">

      {/* HERO */}
      <section className="relative overflow-hidden">
        <div className="absolute right-0 top-0 h-[500px] w-[500px] rounded-full bg-orange-600/10 blur-[130px]" />

        <div className="relative mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:py-32">

          <div className="max-w-4xl">

            <div className="mb-6 inline-flex rounded-full border border-orange-500/20 bg-orange-500/10 px-4 py-2 text-sm text-orange-300">
              Как это работает
            </div>

            <h1 className="text-5xl font-black leading-[1.05] tracking-tight sm:text-6xl lg:text-7xl">
              От идеи
              <span className="block text-orange-600">
                до работающего бизнеса
              </span>
            </h1>

            <p className="mt-8 max-w-3xl text-lg leading-8 text-gray-400 sm:text-xl">
              Не нужно сразу знать всё. Мы двигаемся последовательно:
              разбираем идею, проверяем её, считаем цифры и превращаем
              неопределённость в конкретный план действий
            </p>

          </div>

          {/* MINI STATS */}
          <div className="mt-16 grid max-w-4xl gap-4 sm:grid-cols-3">

            {[
              ["01", "Идея", "с чего начинаем"],
              ["02", "План", "что делаем дальше"],
              ["03", "Запуск", "переходим к действиям"],
            ].map(([number, title, text]) => (
              <div
                key={number}
                className="rounded-3xl border border-white/10 bg-white/[0.035] p-6"
              >
                <div className="text-sm text-orange-500">
                  {number}
                </div>

                <div className="mt-3 text-xl font-bold">
                  {title}
                </div>

                <div className="mt-1 text-sm text-gray-500">
                  {text}
                </div>
              </div>
            ))}

          </div>

        </div>
      </section>

      {/* PROCESS */}
      <section className="border-y border-white/10 bg-[#0E1728]">
        <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8">

          <div className="max-w-2xl">
            <p className="text-sm font-bold uppercase tracking-[0.2em] text-orange-500">
              Наш подход
            </p>

            <h2 className="mt-4 text-4xl font-bold sm:text-5xl">
              Путь состоит из шагов
            </h2>

            <p className="mt-5 text-lg leading-8 text-gray-400">
              Каждый следующий этап опирается на предыдущий.
              Поэтому мы не пытаемся построить весь бизнес за один день
            </p>
          </div>

          <div className="relative mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">

            {steps.map((step) => (
              <div
                key={step.number}
                className="group relative rounded-3xl border border-white/10 bg-[#111a2c] p-7 transition duration-300 hover:-translate-y-2 hover:border-orange-500/30"
              >

                <div className="flex items-start justify-between">
                  <div className="text-4xl">
                    {step.icon}
                  </div>

                  <div className="text-5xl font-black text-white/[0.04]">
                    {step.number}
                  </div>
                </div>

                <h3 className="mt-8 text-2xl font-bold">
                  {step.title}
                </h3>

                <p className="mt-4 leading-7 text-gray-400">
                  {step.text}
                </p>

                <div className="mt-7 h-1 w-10 rounded-full bg-orange-600 transition-all duration-300 group-hover:w-20" />

              </div>
            ))}

          </div>

        </div>
      </section>

      {/* PRINCIPLE */}
      <section className="px-5 py-24 sm:px-8">
        <div className="mx-auto max-w-5xl">

          <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-orange-950/40 via-[#111a2c] to-[#0B1220] p-8 sm:p-12 lg:p-16">

            <p className="text-sm font-bold uppercase tracking-[0.2em] text-orange-500">
              Главный принцип
            </p>

            <h2 className="mt-5 text-4xl font-bold sm:text-5xl">
              Не усложнять то,
              <span className="block text-gray-500">
                что можно сделать понятным
              </span>
            </h2>

            <p className="mt-7 max-w-3xl text-lg leading-8 text-gray-400">
              Бизнес может казаться огромной и сложной системой.
              Но если разбить его на небольшие задачи, становится гораздо
              проще понять, что делать прямо сейчас
            </p>

            <Link
              href="/contact"
              className="mt-9 inline-flex rounded-2xl bg-orange-700 px-7 py-4 font-bold transition hover:-translate-y-1 hover:bg-orange-600"
            >
              Обсудить свою идею →
            </Link>

          </div>

        </div>
      </section>

    </main>
  );
}